-- KC Academy APPLY ALL (existing site profiles)
...
-- Profiles already exist on this project — columns added above.
-- Merged signup trigger: keep site full_name + academy username defaults.
...
drop policy if exists "profiles: edit own username" on public.profiles;
...
drop policy if exists "own redemptions" on public.code_redemptions;
