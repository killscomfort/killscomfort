...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error) {
      ...
      // GoTrue often returns the vague "Error updating user" for email collisions.
      // Re-check so we don't mislabel a real failure on a brand-new address.
      const collided =
      ...
      console.error("[academy/attach-email] updateUserById failed", error);
      return NextResponse.json(
        {
          error:
            "Could not save that email on this guest session. Try a different email, or create an account from Sign up.",
          code: "attach_failed",
        },
        { status: 400 }
      );
